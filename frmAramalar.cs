...
{ 
    public partial class frmAramalar : Form
    {
        ...
        public void dgwPersonelAramaDoldur()
        {
            ...
            da1 = new SqlDataAdapter("SELECT PersonelAdi,PersonelSoyadi,ODT.DemirbasID,DemirbasAdi,AlinanAdet FROM tblPersonel P INNER JOIN tbldepDemirbasAtama ODT ON P.PersonelID=ODT.PersonelID INNER JOIN tblDemirbas D ON D.DemirbasID=ODT.DemirbasID", baglanti);
            ds1 = new DataSet();
        ...
        private void btnAramalarArama_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string ad = txtAramalarAd.Text;
                string soyad = txtAramalarSoyad.Text;
                da = new SqlDataAdapter("SELECT PersonelAdi,PersonelSoyadi,ODT.DemirbasID,DemirbasAdi,AlinanAdet FROM tblPersonel P INNER JOIN tbldepDemirbasAtama ODT ON P.PersonelID=ODT.PersonelID INNER JOIN tblDemirbas D ON D.DemirbasID=ODT.DemirbasID WHERE PersonelAdi='" + ad + "' AND PersonelSoyadi='" + soyad + "'", baglanti);
                ds = new DataSet();
